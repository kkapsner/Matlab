function roi = segment(seg, image, stack)
...
%     image = (image - minV) / (maxV - minV);
    ...
    if (seg.performFilling)
        binImage = Image.fill(binImage, seg.fillingMaxHoleSize);
        %binImage = bwmorph(binImage, 'fill');
    end
    if (seg.performDeadEndRemoving)
        binImage = Image.removeDeadEnds(binImage);
    end
    if (seg.performBridging)
%                 binImage = ~bwmorph(~binImage, 'bridge');
        binImage = ~Image.bridge(~binImage);
    end
    if (seg.performExtrude)
        binImage = ~bwmorph(~binImage, 'dilate', seg.extrudeStrength);
