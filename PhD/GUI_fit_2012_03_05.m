function result = GUI_fit_2012_03_05(data, varargin)
    ...
    hMarkerSlider = zeros(1, markerCount);
    ...
    for i = 1:markerCount
        ...
        hMarkerSlider(i) = uicontrol( ...
            'Style', 'slider', ...
            'Value', result.marker(i), ...
            'Min', 1, ...
            'Max', dataLength, ...
            'String', 'test', ...
            ...'UserData', hMarkerUserData, ...
            'Tag', sprintf('Marker%u', i), ...
            'Callback', @hMarkerCallback, ...
            'SliderStep', [0.01, 0.1],...
            'Parent', f, ...
            'Position', [60 980-30*i 1690 20] ...
        );
        hMarkerUserData = struct('index', i);
        hMarkerUserData.plot =  line( ...
            [result.marker(i) result.marker(i)], ...
            markerY, ...
            'Color', markerColorOrder(i, :), ...
            'Parent', hPlot ...
        );
        hMarkerUserData.point = imline( ...
            hPlot,  ...
            [[result.marker(i); result.marker(i)] markerY'], ...
            ...result.marker(i), data(result.marker(i)), ...
            'PositionConstraintFcn', @markerConstraint...
        );
        hMarkerUserData.point.addNewPositionCallback( ...
            @(pos)set(hMarkerSlider(i), 'Value', pos(1)) ...
        );
        hMarkerUserData.point.setColor(markerColorOrder(i, :));
        
        set(hMarkerSlider(i), 'UserData', hMarkerUserData);
        
        addlistener(hMarkerSlider(i), 'Value', 'PostSet', @hMarkerValueCallback);
    end
    ...
    function hMarkerCallback(hObject, ~)
        userData = get(hObject, 'UserData');
        index = userData.index;
        if (~isnan(index))
            value = get(hObject, 'Value');
            rValue = round(value);
            result.marker(index) = rValue;
            
            pointPos = userData.point.getPosition;
            if pointPos(1) ~= value
                if numel(pointPos) == 2
                    userData.point.setPosition(rValue, data(rValue));
                else
                    userData.point.setPosition([[rValue; rValue] markerY']);
                end
            end
            
            set(userData.plot, 'XData', [value value]);
        end
    end

    function hMarkerValueCallback(~, eventdata)
        hMarker = get(eventdata, 'AffectedObject');
        hMarkerCallback(hMarker, eventdata);
    end

    function pos = markerConstraint(pos)
        if (pos(1) < 1)
            pos(1) = 1;
        elseif (pos(1) > dataLength)
            pos(1) = dataLength;
        end
        pos(1) = round(pos(1));
        if (numel(pos) == 2)
    ...
    function figureResizeCallback(hObject, ~)
        ...
        if (pos(4) < 400)
            outerPos = get(hObject, 'OuterPosition');
            outerPos(2) = outerPos(2) - 400 + pos(4);
            outerPos(4) = outerPos(4) + 400 - pos(4);
            pos(4) = 400;
            set(hObject, 'OuterPosition', outerPos);
        end
        ...
        for index = 1:markerCount
            ...
            set(hMarkerSlider(index), ...
                'Position', [60 y sliderWidth 20] ...
            );
        end
        y = y - 10;
