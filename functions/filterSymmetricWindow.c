                  ...
                  int nrhs, const mxArray *prhs[] )
{
  ...
  /* The input must be a noncomplex double.*/
  if (windowSize[(windowSize[0] < windowSize[1])? 0: 1] != 1) {
    mexErrMsgIdAndTxt( "MATLAB:filterSymmetricWindow:window no vector",
            "Window input must be a vector.");
  }
  windowSize[0] = (windowSize[0] > windowSize[1])? windowSize[0]: windowSize[1];
  ...
  if (dataSize[(dataSize[0] < dataSize[1])? 0: 1] != 1){
      int i;
      ...
      filterSymmetricWindow(filtered,
              window, windowSize[0],
              data, dataSize[(dataSize[0] > dataSize[1])? 0: 1]);
  }
}
